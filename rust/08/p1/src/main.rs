    ...
    for antenna in antennas {
        ...
        for i in 0..positions.len() {
            for j in i+1..positions.len() {
                ...
                let a1x = x1 as i16 - dx;
                let a1y = y1 as i16 - dy;

                let a2x = x2 as i16 + dx;
                let a2y = y2 as i16 + dy;

                if in_bounds(&map, a1x, a1y) {
                    antinode_map[a1y as usize][a1x as usize] = '#';
                }

                if in_bounds(&map, a2x, a2y) {
                    antinode_map[a2y as usize][a2x as usize] = '#';
                }
            }
        }
